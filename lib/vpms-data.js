...
export const resourceDefinitions = {
  owners: {
    ...
    sortBy: "owner_id DESC",
    selectSql:
      "SELECT owner_id, full_name, phone, email, address, created_at FROM owners ORDER BY owner_id DESC",
  ...
  vehicles: {
    ...
    sortBy: "vehicle_id DESC",
    selectSql:
      "SELECT v.vehicle_id, v.owner_id, o.full_name AS owner_name, v.vehicle_number, v.vehicle_type, v.brand, v.model, v.color, v.status, v.created_at FROM vehicles v LEFT JOIN owners o ON o.owner_id = v.owner_id ORDER BY v.vehicle_id DESC",
  ...
  parking_zones: {
    ...
    sortBy: "zone_id DESC",
    selectSql:
      "SELECT zone_id, zone_name, description, base_rate_per_hour, status, created_at FROM parking_zones ORDER BY zone_id DESC",
  ...
  staff: {
    ...
    sortBy: "staff_id DESC",
    selectSql:
      "SELECT staff_id, staff_name, role, phone, username, status, created_at FROM staff ORDER BY staff_id DESC",
  ...
  entries: {
    ...
    selectSql:
      "SELECT e.entry_id, e.vehicle_id, v.vehicle_number, e.slot_id, s.slot_code, e.staff_id, st.staff_name, e.entry_time, e.status FROM entries e LEFT JOIN vehicles v ON v.vehicle_id = e.vehicle_id LEFT JOIN slots s ON s.slot_id = e.slot_id LEFT JOIN staff st ON st.staff_id = e.staff_id ORDER BY e.entry_id DESC",
  ...
  payments: {
    ...
    sortBy: "payment_id DESC",
    selectSql:
      "SELECT p.payment_id, p.fee_id, f.vehicle_id, v.vehicle_number, p.exit_id, x.reference_no AS exit_reference, p.amount_paid, p.payment_mode, p.reference_no, p.paid_at FROM payments p LEFT JOIN fees f ON f.fee_id = p.fee_id LEFT JOIN vehicles v ON v.vehicle_id = f.vehicle_id LEFT JOIN exits x ON x.exit_id = p.exit_id ORDER BY p.payment_id DESC",
  ...
  passes: {
    ...
    sortBy: "pass_id DESC",
    selectSql:
      "SELECT p.pass_id, p.vehicle_id, v.vehicle_number, p.pass_type, p.start_date, p.end_date, p.status, p.pass_fee, p.created_at FROM passes p LEFT JOIN vehicles v ON v.vehicle_id = p.vehicle_id ORDER BY p.pass_id DESC",
  ...
  violations: {
    ...
    sortBy: "violation_id DESC",
    selectSql:
      "SELECT v.violation_id, v.vehicle_id, veh.vehicle_number, v.violation_type, v.fine_amount, v.payment_status, v.issued_by, s.staff_name AS issued_by_name, v.issued_at, v.paid_at FROM violations v LEFT JOIN vehicles veh ON veh.vehicle_id = v.vehicle_id LEFT JOIN staff s ON s.staff_id = v.issued_by ORDER BY v.violation_id DESC",
