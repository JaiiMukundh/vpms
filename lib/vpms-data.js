...
export const staffRoleOptions = [
  { value: "ATTENDANT", label: "Gate Attendant" },
...
export const resourceDefinitions = {
  ...
  staff: {
    ...
    sortBy: "staff_id ASC",
    selectSql:
      "SELECT staff_id, staff_name, CASE role WHEN 'ATTENDANT' THEN 'Gate Attendant' WHEN 'SUPERVISOR' THEN 'Supervisor' ELSE role END AS role_label, phone, username, status, created_at FROM staff ORDER BY staff_id ASC",
    searchFields: ["staff_name", "role_label", "phone", "username"],
    optionLabelFields: ["staff_name"],
    fields: [
  ...
  payments: {
    ...
    selectSql:
      "SELECT p.payment_id, p.fee_id, f.vehicle_id, v.vehicle_number, CASE e.staff_id WHEN 1 THEN 'Arjun Nair' WHEN 2 THEN 'Priya Kulkarni' WHEN 3 THEN 'Vivek Bansal' ELSE e_staff.staff_name END AS entry_staff_name, CASE x.staff_id WHEN 1 THEN 'Arjun Nair' WHEN 2 THEN 'Priya Kulkarni' WHEN 3 THEN 'Vivek Bansal' ELSE x_staff.staff_name END AS exit_staff_name, p.amount_paid, p.payment_mode, p.reference_no, p.paid_at FROM payments p LEFT JOIN fees f ON f.fee_id = p.fee_id LEFT JOIN vehicles v ON v.vehicle_id = f.vehicle_id LEFT JOIN exits x ON x.exit_id = p.exit_id LEFT JOIN entries e ON e.entry_id = f.entry_id LEFT JOIN staff e_staff ON e_staff.staff_id = e.staff_id LEFT JOIN staff x_staff ON x_staff.staff_id = x.staff_id ORDER BY p.payment_id ASC",
    searchFields: ["vehicle_number", "payment_mode", "reference_no", "entry_staff_name", "exit_staff_name"],
    ...
    columns: [
      ...
      column("entry_staff_name", "Entry Staff"),
      column("exit_staff_name", "Exit Staff"),
